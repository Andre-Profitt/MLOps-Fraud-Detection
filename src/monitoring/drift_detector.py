...
class DriftDetector:
    ...
    def detect_feature_drift(
        ...
        if features is None:
            features = [col for col in self.baseline_data.columns 
    ...
    def detect_prediction_drift(
        ...
        chi2_stat, chi2_p_value = stats.chisquare(
            current_counts,
            f_exp=baseline_counts * (len(current_predictions) / len(baseline_predictions))
        )
        ...
        if baseline_probabilities is not None and current_probabilities is not None:
            ...
            ks_stat, ks_p_value = self.calculate_ks_statistic(
    ...
    def should_trigger_retraining(
        ...
        critical_drift = any(
            details['psi'] > 0.25
            for details in feature_drift['features'].values()
        )
