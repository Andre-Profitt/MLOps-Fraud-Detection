...
# Prometheus metrics
prediction_counter = Counter("prediction_requests_total", "Total prediction requests", ["model_version", "prediction"])

prediction_latency = Histogram("prediction_latency_seconds", "Prediction latency in seconds", ["model_version"])

fraud_predictions = Counter("fraud_predictions_total", "Total fraud predictions", ["model_version"])

model_confidence = Histogram("model_confidence_score", "Model confidence scores", ["model_version", "prediction"])
...
def engineer_features(features_dict: Dict) -> pd.DataFrame:
    ...
    # Amount bins
    df["Amount_Bin"] = pd.cut(df["Amount"], bins=[0, 50, 200, 1000, np.inf], labels=[0, 1, 2, 3]).astype(int)
...
async def predict(request: PredictionRequest):
    ...
    try:
        ...
        prediction_counter.labels(
            model_version=model_version, prediction="fraud" if prediction == 1 else "normal"
        ).inc()
...
async def reload_models():
    ...
    try:
        load_models()
        return {"status": "success", "message": "Models reloaded successfully", "timestamp": datetime.now().isoformat()}
    except Exception as e:
